...
async def check_input_rail(text: str, rails=None) -> dict:
    ...
    # NeMo trả về response ở nhiều dạng tuỳ bản nemoguardrails. Ở bản hiện tại, generate_async
    # có thể trả về dict hoặc string.
    response_obj = await rails.generate_async(
        messages=[{"role": "user", "content": text}]
    )
    response = response_obj["content"] if isinstance(response_obj, dict) else response_obj
...
async def check_output_rail(question: str, answer: str, rails=None) -> dict:
    ...
    response_obj = await rails.generate_async(messages=[
