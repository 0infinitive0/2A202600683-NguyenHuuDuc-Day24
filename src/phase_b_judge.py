...
def swap_and_average(question: str, answer_a: str, answer_b: str) -> JudgeResult:
    ...
    swap_map = {"A": "B", "B": "A", "tie": "tie", "A hoặc B hoặc tie": "tie"}
...
def bias_report(judge_results: list[JudgeResult]) -> dict:
    ...
    if total == 0:
        return {"total_judged": 0, "position_bias_rate": 0.0, "verbosity_bias": 0.0, "position_bias_count": 0, "verbosity_details": {}, "interpretation": ""}
...
if __name__ == "__main__":
    ...
    judge_results = []
    judge_labels = []
    print("Running judge on human dataset...")
    for item in human_data:
        jr = swap_and_average(item["question"], item["answer_a"], item["answer_b"])
        ...
        if jr.final_winner == "A":
            ...
            # If tie, let's just pick what human usually wouldn't or matching something.
            # Usually we don't have ties, or we map tie to 0.5. Since label is 1/0, let's say tie = 0.
            judge_labels.append(0)
            
    kappa = cohen_kappa(judge_labels, human_labels)
    print(f"Cohen's κ (real): {kappa:.3f}")

    # --- Bias report ---
    bias = bias_report(judge_results)
    print(f"\nBias report: {bias}")
    
    with open("reports/bias_report.json", "w", encoding="utf-8") as f:
